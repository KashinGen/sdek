    ...
    return (
      ...
      <div className="sm:flex sm:justify-between sm:items-baseline">
        ...
        <div className="text-gray-700">
          {value.toString() || <span className="text-gray-400 italic">Not provided</span>}
        </div>
      </div>
    </div>
    );
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-8">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <h1 className="text-3xl font-bold mb-8 text-gray-900">Delivery Details</h1>

        {renderSection("General Information", generalInfo)}
        {delivery.sender && renderSection("Sender", renderField("Name", delivery.sender.name))}
        {delivery.recipient &&  renderSection("Recipient", renderField("Name", delivery.recipient.name))}
        {delivery.from_location &&  renderSection("From Location", renderLocation(delivery.from_location))}
        {delivery.to_location && renderSection("To Location", renderLocation(delivery.to_location))}
        {delivery.packages.length > 0 &&  renderSection("Packages", renderPackages)}

        {delivery.statuses.length > 0 && renderSection("Status History", (
          <ul className="divide-y divide-gray-200">
            {delivery.statuses.map((status, index) => (
              <li key={index} className="py-2">
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-gray-700">{status.name}</span>
                  <span className="text-gray-500 text-sm">{formatDate(status.date_time)}</span>
                </div>
        ...
        {delivery.delivery_detail && renderSection("Delivery Details", (
          <>
            {renderField("Delivery Sum", delivery.delivery_detail.delivery_sum)}
            {renderField("Total Sum", delivery.delivery_detail.total_sum)}
            {renderField("Delivery VAT Rate", delivery.delivery_detail.delivery_vat_rate)}
            {renderField("Delivery VAT Sum", delivery.delivery_detail.delivery_vat_sum)}
            {renderField("Delivery Discount Percent", delivery.delivery_detail.delivery_discount_percent)}
            {renderField("Delivery Discount Sum", delivery.delivery_detail.delivery_discount_sum)}
          </>
        ))}
      </div>
    </div>
